...
# Check if the input Deployment's containers have requested CPU resources and return a message
check[msg] {
...
# Check if the input Deployment's containers have not requested CPU resources and return a message
check[msg] {
  input.kind == "Deployment"
  container := input.spec.containers[_]
...
# Check if the input Deployment's containers have requested memory resources and return a message
check[msg] {
  input.kind == "Deployment"
  container := input.spec.containers[_]
  container_has_memory_request(container)
...
# Check if the input Deployment's containers have not requested memory resources and return a message
check[msg] {
  input.kind == "Deployment"
  container := input.spec.containers[_]
