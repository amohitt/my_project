...
# Check if the input CronJob's containers have requested CPU resources and return a message
check[msg] {
  input.kind == "CronJob"
...
# Check if the input CronJob's containers have not requested CPU resources and return a message
check[msg] {
  input.kind == "CronJob"
  container := input.spec.containers[_]
...
# Check if the input CronJob's containers have requested memory resources and return a message
check[msg] {
  input.kind == "CronJob"
  container := input.spec.containers[_]
  container_has_memory_request(container)
...
# Check if the input CronJob's containers have not requested memory resources and return a message
check[msg] {
  input.kind == "CronJob"
  container := input.spec.containers[_]
